...
subroutine readinput
   ! Read in the control parameters from pn.in,
   ! and set default values if not specified in the pn.in
   !
   ! Constructed on 22/4/2010 by QS.Wu
   ! Modified for Simphony on 26/10/2024 by Francesc Ballester
...
260   continue
   ...
   if (cpuid.eq.0) then
      ...
      write(outfileindex, '(a)')"Rotated POSCAR by SURFACE card in pn.in by Simphony"
...
subroutine writeout_poscar(cell, poscarname)
   ...
   if (cpuid.eq.0) then
      ...
      write(outfileindex, '(a)')"POSCAR for generated by Simphony"
      write(outfileindex, '(a)')"1.0"
...
subroutine generate_slab_poscar(cell)
   ...
   if (cpuid.eq.0) then
      ...
      write(outfileindex, '(a)')"POSCAR for slab defined by SURFACE card and Nslab and Vacuum_thickness_in_Angstrom in pn.in by Simphony"
      write(outfileindex, '(a)')"1.0"
      write(outfileindex, '(3f12.6)') R1/Angstrom2atomic
      write(outfileindex, '(3f12.6)') R2/Angstrom2atomic
