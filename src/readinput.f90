!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! TODO: 
!   - Delete unnecessary routines
!   - Change units from eV to THz 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!


subroutine readinput
   ! Read in the control paramters from pn.in,
   ! and set default values if not specified in the pn.in
   !
   ! Constructed on 22/4/2010 by QS.Wu
   ! Modified for Phonnier on 26/10/2024 by Francesc Ballester

   use wmpi
   use para
   implicit none

   character*12 :: fname='pn.in'
...
!===============================================================================================================!
   ...
   !   write(stdout, *)' ERROR: Particle should equal either "electron"' , &
   !      '"phonon", or "photon"'
   !   stop
   !endif
...
!> CONTROL namelist
!===============================================================================================================!
   !> TODO: Commented calculations should be removed, only available for electrons and not for phonons. Or might be added afterwards.
   BulkBand_calc         = .FALSE.
   ...
   if (stat/=0) then
      ...
      write(*, *)"And please make sure that the spelling is correct."
...
!===============================================================================================================!
   !!!!!!!!!!!
   ! For phonons, we consider always projectors px, py and pz, so this card is now deprecated
   !!!!!!!!!!!
...
!102 continue
   ...
   !> create wannier centers
   allocate(Origin_cell%nprojs(Origin_cell%Num_atoms))
   allocate(Origin_cell%proj_name(3, Origin_cell%Num_atoms))
   do ia=1, Origin_cell%Num_atoms
        Origin_cell%nprojs(ia) = 3
        Origin_cell%proj_name(1, ia) = 'px'
        Origin_cell%proj_name(2, ia) = 'py'
        Origin_cell%proj_name(3, ia) = 'pz'
   end do
...
110 continue
   ...
   if (lfound) then
      if (cpuid==0) then
         write(stdout, *)" "
         write(stdout, *)">> Wannier centers from pn.in, in unit of unit lattice vectors"
...
260   continue
   ...
   if (abs(abs(cell_volume2)-abs(Origin_cell%CellVolume))> 0.001d0.and.cpuid==0) then
      ...
      write(stdout, '(a)')" However, don't worry, Phonnier will help you to find a suitable rotation matrix."
...
104 continue
   if (.not.lfound .and. (BulkBand_line_calc)) then
      stop 'ERROR: please set KPATH_BULK for bulk band Origin_cell%structure calculation'
   endif
...
subroutine writeout_poscar(cell, poscarname)
   ...
   if (cpuid.eq.0) then
      ...
      write(outfileindex, '(a)')"POSCAR for generated by Phonnier"
      write(outfileindex, '(a)')"1.0"
...
subroutine generate_slab_poscar(cell)
   ...
   if (cpuid.eq.0) then
      ...
      write(outfileindex, '(a)')"POSCAR for slab defined by SURFACE card and Nslab and Vacuum_thickness_in_Angstrom in pn.in by Phonnier"
      write(outfileindex, '(a)')"1.0"
      write(outfileindex, '(3f12.6)') R1/Angstrom2atomic
      write(outfileindex, '(3f12.6)') R2/Angstrom2atomic
